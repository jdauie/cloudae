...
function IntervalMapIndex(index, ratio) {
	this.index = index;
	this.stretchRatio = ratio;
};

function IntervalMap(stretch, desiredDestinationBins, scaleDesiredBinsToStretchRange) {
	...
	this.getInterval = function(actualValue)
	{
	...
	this.getIntervals = function()
	{
		...
		for (var i = start; i < stretchStart; i++)
			intervals.push(new IntervalMapIndex(i, 0.0));

		for (var i = stretchStart; i <= stretchEnd; i++)
			intervals.push(new IntervalMapIndex(i, (i - stretchStart) * inverseStretchRange));

		for (var i = stretchEnd + 1; i < end; i++)
			intervals.push(new IntervalMapIndex(i, 1.0));

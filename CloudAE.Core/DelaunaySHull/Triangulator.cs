...
namespace CloudAE.Core.Delaunay
{
    class Triangulator
    {
        ...
        private void Analyse(Vertex[] suppliedPoints, Hull hull, List<Triad> triads, bool rejectDuplicatePoints, bool hullOnly)
        {
            ...
            if (rejectDuplicatePoints)
            {
				...
				for (int sourceIndex = 0; sourceIndex < nump; sourceIndex++)
				{
					if ((sourceIndex == nump - 1) || !points[sortedIndices[sourceIndex]].Equals(points[sortedIndices[sourceIndex + 1]]))
					{
						if (sourceIndex > destinationIndex)
						{
							sortedIndices[destinationIndex] = sortedIndices[sourceIndex];
							distance2ToCentre[destinationIndex] = distance2ToCentre[sourceIndex];
						}

						++destinationIndex;
					}
				}
				nump = destinationIndex;
            }

			Debug.WriteLine("{0}/{1} points rejected as duplicates", points.Length - nump, points.Length);
            ...
            Vertex centre = new Vertex(circumCentreX, circumCentreY, 0);
...
#warning I added this so it would complete, but results are incorrect
					if (e1 < 0) e1 = 0;
        ...
        /// <returns></returns>
        public List<Vertex> ConvexHull(Vertex[] points)
        {
        ...
        /// <returns></returns>
        public List<Vertex> ConvexHull(Vertex[] points, bool rejectDuplicatePoints)
        {
            Hull hull = new Hull();
            ...
            foreach (HullVertex hv in hull)
                hullVertices.Add(new Vertex(hv.x, hv.y, hv.z));
        ...
        public List<Triad> Triangulation(Vertex[] points)
        {
        ...
        /// <param name="rejectDuplicatePoints">Whether to omit duplicated points</param>
        /// <returns></returns>
        public List<Triad> Triangulation(Vertex[] points, bool rejectDuplicatePoints)
        {
            List<Triad> triads = new List<Triad>();
            Hull hull = new Hull();

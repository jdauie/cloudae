...
namespace CloudAE.Core.Compression
{
	static class QuickLZ
	{
		...
		public static int compress(byte[] source, int length, byte[] destination, int level)
		{
			int src = 0;
			...
			while (src <= last_matchstart)
			{
				if ((cword_val & 1) == 1)
				{
					if (src > length >> 1 && dst > src - (src >> 5))
					{
						...
						System.Array.Copy(source, 0, destination, DEFAULT_HEADERLEN, length);
						return length;
					}
		...
		public static int Decompress(byte[] source, int length, byte[] destination)
		{
			...
			if ((source[0] & 1) != 1)
			{
				...
				System.Array.Copy(source, headerLen(source), destination, 0, size);
				return size;
			}

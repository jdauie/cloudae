...
namespace Jacere.Core
{
	...
	public class CachedColorRamp
	{
		...
		public CachedColorRamp(ColorRamp ramp, StretchBase stretch, int desiredDestinationBins)
		{
			...
			foreach (var interval in m_map.GetIntervals())
			    m_bins[interval.Index] = ramp.GetColor(interval.StretchRatio).ToArgb();
	...
	/// Stretched for a specified size and input range (including std dev stretch).
	/// </summary>
	public class CachedColorRampOld
	{
		...
		public CachedColorRampOld(ColorRamp ramp, int min, int max, QuantizedStatistics stats, bool useStdDevStretch, int desiredDestinationBins)
		{
